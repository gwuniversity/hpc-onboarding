...
Once your script has been adjusted for your needs, save your edits, type `sbatch [filepath]` and press enter to send your job to the queue via SLURM, where \[filepath\] is the file location of your submission script. You will get a job ID if it’s been successfully submitted; keep note of it, in case you need to file a ticket for a problem you can’t diagnose. From there, you can check your job as it’s pending or running using `squeue` or after it's done with `sacct`.
