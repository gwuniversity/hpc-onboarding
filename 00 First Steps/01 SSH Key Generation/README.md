...
5. If done properly, you should now be prompted to enter a passphrase to lock your SSH key pair. Type your desired passphrase in, then hit **Enter**. You should then be prompted to re-enter your passphrase; retype your passphrase and press **Enter** once done.  
   ...
   2. **Note:** The passphrase will not display as you type it. This is intentional--the same will be true when you sign into the system and need to input your passphrase. This is done to avoid screen-readers from scraping your passphrase and making your key pair less secure.  
...
Once the passphrase has been accepted, you will then be prompted for your two-factor authentication (2FA) code. If this is your first time signing into an HPC cluster, please go to the next folder on the GitHub here ("02 Initial Login + 2FA Setup") for instructions on initialization of this feature. **Note:** All HPC users at GW are required to set up 2FA to use Pegasus, Cerberus and other clusters offered by RTS. Said 2FA is separate from the 2FA required for logging in to services such as Gmail, GWeb and more.
